...
\begin{frame}[plain]{}
   \begin{block}{Objectives}
      \begin{enumerate}
         ...
         \IfStrEq{\modo}{mcyte}{\item Introduce Scikit-Learn API for supervised algorithms}{}
      \end{enumerate} 
   \end{block}
...
\subsection{Crash introduction to Scikit-Learn}
\begin{frame}{Introduction}{Crash introduction to Scikit-Learn}
    \begin{columns}
 	   \column{.50\textwidth}
        Basic workflow in Scikit-Learn:
        \begin{enumerate}
    	\item Choose a class of model
    	\item Choose hyperparameters
        \item Arrange data
        \item Fit the model (\texttt{fit()})
        \item Evaluate the model
            \begin{itemize}
            \item For supervised learning, \texttt{predict}
            \item For unsupervised learning, \texttt{transform}
            \end{itemize}
        \end{enumerate}

 	   \column{.50\textwidth}
    \centering \includegraphics[width=\linewidth]{figs/scikit.png}\\
    \end{columns}
    \bigskip
    \centering
    \href{https://github.com/dfbarrero/dataCourse/blob/master/mlfoundations/scikit-learn.ipynb}{(Notebook)}
\end{frame}

\subsection[Generalization]{Generalization, overfitting and underfitting}
\begin{frame}{Introduction}{Generalization, overfitting and underfitting}
...
\begin{frame}{k-Nearest Neighbors}{kNN regression (II)}
    ...
    \begin{itemize}
        ...
        \item With large $k$ values, fit is smoother
    \end{itemize}
\end{frame}
...
\begin{frame}{k-Nearest Neighbors}{Summary}
	\begin{center}
	\begin{tabular}{cp{2cm}p{4.5cm}}\hline
...
\begin{frame}{Linear models}{Linear regression}
    Different learn algorithms for linear models
    \begin{itemize}
...
\begin{frame}{Linear models}{Ridge regression}
	Ridge regression (or L2 regularization) adds a new term to the cost function
    \begin{equation*}
...
\IfStrEq{\modo}{mcyte}{
    ...
    \begin{frame}{Linear models}{Scikit-learn: Linear}
    ...
    \begin{frame}{Linear models}{Scikit-learn: Ridge}
    ...
    \begin{frame}{Linear models}{Scikit-learn: Lasso}
    ...
    \begin{frame}{Linear models}{Scikit-learn: ElasticNet}
...
\begin{frame}{Decision Trees}{Building decision trees (III)}
    We need a measure of `impurity': Gini and entropy

    \begin{itemize}
    \item Let $C$ be the number of classes and $p_i$ is the probability of class $i$ in the dataset $S$
        \item Gini: $G(S) = 1 - \sum_i^C p_i^2$
        \item Entropy: $H(S) = - \sum_i^C p_i \log_2 p_i$
    \end{itemize}

    Both measures are used to learn trees
    \begin{itemize}
        \item Gini is faster
        \item Entropy is preferred with unbalanced datasets
    \end{itemize}
\end{frame}
...
\begin{frame}{Support Vector Machines}{Understanding SVMs (I)}
    ...
    \begin{itemize}
        ...
        \item Low $\gamma$, lower complexity
    \end{itemize}
...
\begin{frame}[fragile]{Other approaches (I)}
    \begin{center}
         \begin{columns}
 	        \column{.5\textwidth}
        \centering Semi-supervised learning\\
    	\begin{tabular}{cccccc}\hline
    	 	$f_1$     & $f_2$     & $\cdots$ & $f_n$     & $y$\\\hline
    	 	$a_{1,1}$ & $a_{2,1}$ & $\cdots$ & $a_{n,1}$ &  \\
    	 	$a_{1,2}$ & $a_{2,2}$ & $\cdots$ & $a_{n,2}$ & $y_1$ \\
    	 	$a_{1,3}$ & $a_{2,3}$ & $\cdots$ & $a_{n,3}$ &  \\
    	 	$a_{1,4}$ & $a_{2,4}$ & $\cdots$ & $a_{n,4}$ & $y_2$ \\
    	 	$a_{1,5}$ & $a_{2,5}$ & $\cdots$ & $a_{n,5}$ &  \\
    	 	\hline
    	\end{tabular}

 	        \column{.5\textwidth}
        \centering Weakly-supervised learning\\
    	\begin{tabular}{cccccc}\hline
    	 	$f_1$     & $f_2$     & $\cdots$ & $f_n$     & $y$\\\hline
    	 	$a_{1,1}$ & $a_{2,1}$ & $\cdots$ & $a_{n,1}$ & $y_1 + \epsilon$ \\
    	 	$a_{1,2}$ & $a_{2,2}$ & $\cdots$ & $a_{n,2}$ & $y_2 + \epsilon$ \\
    	 	$a_{1,3}$ & $a_{2,3}$ & $\cdots$ & $a_{n,3}$ & $y_3 + \epsilon$ \\
    	 	$a_{1,4}$ & $a_{2,4}$ & $\cdots$ & $a_{n,4}$ & $y_4 + \epsilon$ \\
    	 	$a_{1,5}$ & $a_{2,5}$ & $\cdots$ & $a_{n,5}$ & $y_5 + \epsilon$ \\
    	 	\hline
    	\end{tabular}
        \end{columns}
...
\begin{frame}[fragile]{Other approaches (II)}
    \begin{center}
        Federated learning\\
    	...
    	\begin{tabular}{cccccc}\hline
    	 	$f_1$     & $f_2$     & $\cdots$ & $f_n$     & $y$\\\hline
    	 	$a'_{1,1}$ & $a'_{2,1}$ & $\cdots$ & $a'_{n,1}$ & $y'_1$ \\
    	 	$a'_{1,2}$ & $a'_{2,2}$ & $\cdots$ & $a'_{n,2}$ & $y'_2$ \\
    	 	$a'_{1,3}$ & $a'_{2,3}$ & $\cdots$ & $a'_{n,3}$ & $y'_3$ \\
    	 	$a'_{1,4}$ & $a'_{2,4}$ & $\cdots$ & $a'_{n,4}$ & $y'_4$ \\
    	 	$a'_{1,5}$ & $a'_{2,5}$ & $\cdots$ & $a'_{n,5}$ & $y'_5$ \\
    	 	\hline
    	\end{tabular}
 	\end{center}
\end{frame}
