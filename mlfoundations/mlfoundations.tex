...
\begin{frame}{The data analysis process}{Machine Learning (I)}
	Our goal is to train a \alert{model}
		\begin{itemize}
		\item Model: representation of a pattern in a dataset
			\begin{itemize}
				\item also named \alert{classifier} or \alert{regressor} or \alert{predictor}, depending on the context
			\end{itemize}
		\item Models are trained with data
		\item We want models able to \alert{generalize} to unseen data
		\item \alert{Prediction}: using a model with unseen data
		\end{itemize}
