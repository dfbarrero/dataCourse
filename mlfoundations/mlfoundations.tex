...
\usetikzlibrary{decorations.text} % vocabulary.tex
...
\begin{frame}{Introduction}{The alphabet soup of data analysis (I)}
	\begin{columns}
 	   %\column{.10\textwidth}
 	   \column{.60\textwidth}
	    \begin{center}
...
%\begin{frame}{Introduction}{The alphabet soup of data analysis (II)}
%	\begin{columns}
% 	   %\column{.10\textwidth}
% 	   \column{.60\textwidth}
%	    \begin{center}
%            \includegraphics[width=0.6\linewidth]{figs/datascience.png}\\
%            \tiny{\href{https://towardsdatascience.com/roadmap-to-becoming-a-data-scientist-part-1-maths-2dc9beb69b27/}{(Source)}}
%	    \end{center}
%    \end{columns}
%\end{frame}
...
\begin{frame}{The data analysis process}{The big picture}
	...
	\begin{columns}
		...
		\begin{enumerate}
			...
			\item \alert{Preprocessing}: Selection, cleaning and transformation
			\item Machine Learning
...
\begin{frame}{The data analysis process}{Selection, cleaning and transformation (I)}
	...
	Goal: Prepare data for ML
		...
		\item It involves data selection, cleaning and transformation
		\end{itemize}
...
\begin{frame}[fragile]{The data analysis process}{Selection, cleaning and transformation (II)}
	\begin{exampleblock}{Example: Bank data base (tabular data)}
	 \vspace{-0.3cm}
	\begin{center}
	...
	\begin{exampleblock}{Example: Robot sensors (multivariable time series}
	 \vspace{-0.3cm}
	\begin{center}
	\begin{tabular}{cccccc}\hline
...
\begin{frame}[fragile]{The data analysis process}{Selection, cleaning and transformation (IV)}
	\begin{exampleblock}{Example: Natural Language Processing (bag-of-words representation)}
	 \vspace{-0.2cm}
	\begin{enumerate}
		\item Original text\\
		\begin{lstlisting}[basicstyle=\scriptsize]
...
\begin{frame}{The data analysis process}{Machine Learning (I)}
	Our goal is to train a \alert{model}
	\begin{itemize}
		\item ... also named \textbf{classifier} or \textbf{regressor} or \textbf{predictor}, depending on the context
		\item Models are learnt by learning algorithms
			\begin{itemize}
				\item Models are represented by \alert{parameters}
				\item Learning algorithms are configured with \alert{hyperparameters}
				\item Number of parameters highly correlated with learning capacity
			\end{itemize}
	\end{itemize}
	...
	\begin{itemize}
		\item Prediction: using a model with unseen data
		\item Models must be able to \alert{generalize} to unseen data
	\end{itemize}
\end{frame}

\begin{frame}{The data analysis process}{Machine Learning (II)}
...
\begin{frame}{The data analysis process}{Learning evaluation (I)}
	...
	\begin{itemize}
		\item Models must perform well on new data $\Rightarrow$ generalization
		\item Evaluation must, always, be performed on unseen data
	\end{itemize}
...
\begin{frame}{The data analysis process}{Learning evaluation (II)}
	\begin{columns}
		...
		\begin{itemize}
			...
			\item Used to get a reliable assessment of model performance
		\end{itemize}
